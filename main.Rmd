...
factor.features <- names(train)[which(sapply(train, is.factor))]
lost.values.features.factor <- dplyr::intersect(factor.features, lost.values.features)

plots <- lapply(lost.values.features.factor, function(feature) {
    categories <- train[, feature]
    ggplot(data = train, aes(x = feature, y = log(SalePrice), fill = categories)) +
        geom_boxplot()
})
