...
```{r}
features.class <- sapply(names(full.set.transformed), function(feature) {
    class(full.set.transformed[, feature])
})
...
features.class.numeric.skewed <- sapply(features.class.numeric, function(feature) {
    e1071::skewness(full.set.transformed[, feature], na.rm = TRUE)
})
...
for (feature in names(features.class.numeric.skewed)) {
    box.cox = caret::BoxCoxTrans(full.set.transformed[, feature], lambda = 0.15)
    full.set.transformed[, feature] = predict(box.cox, full.set.transformed[, feature])
}
```
...
predictAndEvaluate <- function(model, validation.set) {
    ...
    rmse(validation.set$SalePrice, validation.prediction) # 0.1344078
}
```
...
```{r}
model.ensemble <- ensembleTrain(train.processed.partition.train2)
```

```{r}
model.ensemble # 0.1322569
predictAndEvaluate(model.ensemble, train.processed.partition.validation2) # 0.1230385
```
